...
if geos == 3 :
    zone = 'NORTH_INDO'
    latmin, latmax =  0, 15.
    lonmin, lonmax = 105., 130.
elif geos == 4 :
    zone = 'PACIFIQUE_OUEST'
    latmin, latmax =  0., 10.
    lonmin, lonmax = 140., 170.


name_text = '/cnrm/tropics/commun/DATACOMMUN/WAVE/NO_SAVE/DATA/ANALYSIS_TOOCAN/Distribution/life_data_MCS_' + zone + '_JAS.txt'
...
for name_phase in phase_sel :
    ...
    for wave in wave_SEL :	
        ...
        ds = ds.isel(time = ds.time.dt.month.isin([7, 8, 9]))
        ds = ds.phase.load()
        ...
        p_distri_G1, p_distri_G2, nb_MCS_1, nb_MCS_2, surface_G1, TbMin_G1, surface_G2, TbMin_G2 = \
                            calcul_Proba_wave(ds, ds_T_I_G1, ds_T_E_G1, data_MCSselection_G1, ds_T_I_G2, ds_T_E_G2, data_MCSselection_G2, 'init', name_phase, iregion)

        ### save numpy array
        save_file = np.stack((nb_occu, p_distri_G1, p_distri_G2, surface_G1, TbMin_G1, surface_G2, TbMin_G2))
        outfile = folder_out + zone + '_nbMCS_' + wave + '_' + phase_p[k] + '_init_JAS.npy'
        np.save(outfile, save_file)

        file_text = open(name_text, 'a')
        str_file = 'Pour JAS ' + name_phase + ' ' + zone + ' et ' + wave + ' Nb MCS <5h = ' + str(nb_MCS_1) + ' Nb MCS > 5H = ' + str(nb_MCS_2) + '\n'
        ...
        ### save numpy array
        save_file = np.stack((nb_occu, p_distri_G1, p_distri_G2, surface_G1, TbMin_G1, surface_G2, TbMin_G2))
        outfile = folder_out + zone + '_nbMCS_' + wave + '_' + phase_p[k] + '_life_JAS.npy'
        np.save(outfile, save_file)
    k = k + 1
