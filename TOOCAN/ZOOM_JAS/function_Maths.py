...
def compute_distribution_init(da, ds_T, data_MCSselection):
    ...
    for iMCS in np.arange(0,len(data_MCSselection),1):
        _time = ds_T.isel(time = iMCS).time.values
        if (((_time.astype('datetime64[M]').astype(int) % 12 + 1) == 7) | ((_time.astype('datetime64[M]').astype(int) % 12 + 1) == 8) | ((_time.astype('datetime64[M]').astype(int) % 12 + 1) == 9)):
...
def compute_distribution_life(da, ds_T, data_MCSselection, iregion):
    ...
    ds_precip = ds_precip.isel(time = ds_precip.time.dt.month.isin([7, 8, 9]))
