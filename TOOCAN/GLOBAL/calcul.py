...
def calcul_Proba_wave(ds, ds_T_I_G1, ds_T_E_G1, data_MCSselection_G1, ds_T_I_G2, ds_T_E_G2, data_MCSselection_G2, condition, varFiltre):
    ...
    if condition == 'init':
        ...
        distri_G1, nb_MCS_G1, surface_G1, TbMin_G1 = np.zeros(32, dtype = np.int32), np.zeros(32, dtype = np.int32), np.zeros(32, dtype = np.int32), np.zeros(32, dtype = np.int32) #compute_distribution_init(ds, ds_T_I_G1, data_MCSselection_G1)
        ...
        # # Initialisation des MCS
        ## Pour les MCS inférieur à 5h ##
        distri_G1, nb_MCS_G1 = compute_distribution_life(ds, ds_T_I_G1, data_MCSselection_G1)
        ## Pour les MCS supèrieur à 5h ##
        distri_G2, nb_MCS_G2 = compute_distribution_life(ds, ds_T_I_G2, data_MCSselection_G2)
