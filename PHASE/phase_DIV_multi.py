...
wave = ['d_Kelvin','d_Rossby','d_TD','d_MJO','d_MRG','d_WIG','d_EIG']
outdir = '/cnrm/tropics/commun/DATACOMMUN/WAVE/NO_SAVE/DATA/PHASE/divergence/850/'
indir_data = '/cnrm/tropics/commun/DATACOMMUN/WAVE/NO_SAVE/DATA/FILTERED_ANOMALY/d/reduce/'
coeff = 1
level =[850]

for l in level :
    ds = xr.open_mfdataset(indir_data + '*z' + str(l) + '*.nc', parallel = True)
    ...
    for y in year:
        for w in wave:
            ...
            ds_netcdf.to_netcdf(outdir + 'phase_z' + str(l) + '_' + w + '_' + str(y) + '.nc')
