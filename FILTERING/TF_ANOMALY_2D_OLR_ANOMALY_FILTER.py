...
cluster = LocalCluster(processes=False, n_workers=1, threads_per_worker=32, silence_logs='error')
...
filenames = np.arange(2001,2019,1)
var = 'OLR'
units = "W/m^2"
###################################################################
###################################################################
...
def deletebug(da, ds):
    ...
    _da = da.real.values + da.conjugate().real.values
    da = xr.DataArray(_da,
                       dims=("time","lat","lon"), 
                       coords={"time":ds.time,
                               "lat":ds.lat,
...
for f in filenames:
    ...
    ####---- Block to SYM/ASY ----####
    ################################################
    da = decompose2SymAsym(ds['OLR_ano'])

    x_wintap = da.chunk({"time" : -1, "lat": 1}).load()
    tcwvhat = xrft.fft(x_wintap, dim=['time','lon'], 
                       true_phase=False, true_amplitude=True)


    z_symmetric = tcwvhat.where( tcwvhat.lat < 0 , drop = False)
    z_symmetric = z_symmetric.transpose("lat","freq_lon","freq_time")
    z_symmetric[int(z_symmetric.lat.size/2):,...] = np.flip(z_symmetric[:int(z_symmetric.lat.size/2),...].values,0)
    
    
    z_antisymmetric = tcwvhat.where( tcwvhat.lat > 0 , drop = False)
    z_antisymmetric = z_antisymmetric.transpose("lat","freq_lon","freq_time")
    z_antisymmetric[:int(z_antisymmetric.lat.size/2),...] = np.flip(z_antisymmetric[int(z_antisymmetric.lat.size/2):,...].values,0)

    z_final = xr.concat([z_symmetric, z_antisymmetric], "component")
    z_final = z_final.assign_coords({"component":["symmetric","antisymmetric"]})

    ### Change z_symmetric or z_antisymmetric
    _tcwvhat = z_symmetric.copy()
    ################################################################
    ################################################################
    ...
    _dahat_Kev, da_Kevfilter = kelvinfilter(_tcwvhat, fmin = -1/tMin, fmax = -1/tMax, kmax = kMax, kmin = kMin, hmin=hMin, hmax=hMax)
    da_Kevfilter = da_Kevfilter.transpose("time","lat","lon")
    ...
    _dahat_Ross, da_Rossfilter = erfilter(_tcwvhat, fmin = -1/tMin, fmax = -1/tMax, kmax = kMax, kmin = kMin, hmin=hMin, hmax=hMax)
    da_Rossfilter = da_Rossfilter.transpose("time","lat","lon")
    ...
    # hMax = mis
    _dahat_TD, da_TDfilter = mrgfilter(_tcwvhat, fmin = -1/2.5, fmax = -1/7, kmax = kMax, kmin = kMin, hmin = 100)
    ...
    # ds_NetCdf = da_WIGfilter.to_dataset(name = var + '_WIG')
    ...
    ds_NetCdf.to_netcdf(outdir_data + 'OLR_CERES_3H_' + str(f) +'_SYM.nc', unlimited_dims='time')
